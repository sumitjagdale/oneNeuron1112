...
def save_plot(df, file_name, model):
    ...
    :param file_name: The path to save the plot
    :param model: trained models
   
    """
   
    def _create_base_plot(df):
        logging.info("Creating the baseplot")
    ...
    def _plot_decision_regions(X, y, classfier, resolution=0.02):
        ...
        x1 = X[:, 0] # X axis (All the rows 0th column)
        x2 = X[:, 1] # Y axis (All the rows 1st column)
        x1_min, x1_max = x1.min() -1 , x1.max() + 1
        x2_min, x2_max = x2.min() -1 , x2.max() + 1  

        xx1, xx2 = np.meshgrid(np.arange(x1_min, x1_max, resolution), 
                            np.arange(x2_min, x2_max, resolution)) # Creating test values for x1 & x2
       # print(xx1)
        #print(xx1.ravel())
        ...
        plotPath = os.path.join(plot_dir, file_name) # plot path/filename
